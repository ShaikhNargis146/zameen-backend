...
import { sweepPlanSubscriptions } from "./modules/commerce/commerce.service.js";
...
const PLAN_SUBSCRIPTION_SWEEP_INTERVAL_MS = 15 * 60 * 1000;
const runPlanSubscriptionSweep = () => {
  sweepPlanSubscriptions().catch(error =>
    logger.error(`plan subscription sweep failed: ${error.message}`)
  );
};
...
app.listen(port, "0.0.0.0", err => {
  ...
  runPlanSubscriptionSweep();
  setInterval(runPlanSubscriptionSweep, PLAN_SUBSCRIPTION_SWEEP_INTERVAL_MS);
});
