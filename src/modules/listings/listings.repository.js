...
export const publishedDetail = id =>
  run(
    "oneOrNone",
    `SELECT l.id, l.listing_code AS "listingCode", l.title, l.description, l.transaction_type AS "transactionType", l.price_amount_minor AS "priceAmountMinor", l.currency, l.is_negotiable AS "isNegotiable", l.published_at AS "publishedAt", p.id AS "propertyId", p.public_code AS "propertyCode", pt.code AS "propertyTypeCode", pt.name AS "propertyType", lut.code AS "landUseTypeCode", d.area_value AS "areaValue", au.code AS "areaUnit", d.area_sqft AS "areaSqft", d.frontage_m AS "frontageM", d.road_width_m AS "roadWidthM", d.facing, d.is_corner_plot AS "isCornerPlot", loc.name AS "locationName", pc.code AS pincode, pl.landmark, CASE WHEN pl.show_exact_location THEN ST_Y(pl.coordinates::geometry) END AS latitude, CASE WHEN pl.show_exact_location THEN ST_X(pl.coordinates::geometry) END AS longitude, s.readiness_score AS "scannerScore", s.missing_items AS "scannerMissingItems" FROM marketplace.listings l JOIN land.properties p ON p.id = l.property_id AND p.deleted_at IS NULL JOIN land.property_types pt ON pt.id = p.property_type_id LEFT JOIN land.land_use_types lut ON lut.id = p.land_use_type_id LEFT JOIN land.property_land_details d ON d.property_id = p.id LEFT JOIN land.area_units au ON au.id = d.area_unit_id LEFT JOIN land.property_locations pl ON pl.property_id = p.id LEFT JOIN geo.locations loc ON loc.id = pl.location_id LEFT JOIN geo.postal_codes pc ON pc.id = pl.postal_code_id LEFT JOIN marketplace.v_listing_scanner s ON s.listing_id = l.id WHERE l.id = $1 AND l.status = 'PUBLISHED' AND l.review_status = 'APPROVED' AND l.deleted_at IS NULL`,
    [id]
  );
export const media = propertyId =>
  run(
    "any",
    `SELECT id, media_type AS "mediaType", storage_key AS "storageKey", thumbnail_storage_key AS "thumbnailStorageKey", sort_order AS "sortOrder", is_cover AS "isCover" FROM land.property_media WHERE property_id = $1 AND deleted_at IS NULL ORDER BY sort_order`,
    [propertyId]
  );
...
export const approve = id =>
  run(
    "oneOrNone",
    `UPDATE marketplace.listings SET review_status = 'APPROVED', status = 'INACTIVE', approved_at = now(), rejection_reason = NULL WHERE id = $1 AND review_status = 'PENDING' AND deleted_at IS NULL RETURNING id`,
    [id]
  );
