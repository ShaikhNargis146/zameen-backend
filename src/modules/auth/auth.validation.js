...
const optionalDeviceValue = (value, field) => {
  ...
  if (result.length > 255) {
    const message = `${field} must be at most 255 characters.`;
...
export const otpRequest = body => {
  ...
  if (!channels.has(channel))
    throw new HttpError(400, "VALIDATION_ERROR", "channel must be SMS or EMAIL.", [
  ...
  if (!purposes.has(purpose))
    throw new HttpError(400, "VALIDATION_ERROR", "purpose is invalid.", [
      { field: "purpose", message: "purpose is invalid." }
    ]);
  const phone =
  ...
  if (channel === "SMS" && !/^\+?\d{10,15}$/.test(phone.replace(/\s/g, "")))
    throw new HttpError(400, "VALIDATION_ERROR", "destination must be a valid phone number.", [
      { field: "destination", message: "destination must be a valid phone number." }
    ]);
  if (channel === "EMAIL" && !emailPattern.test(destination))
    throw new HttpError(400, "VALIDATION_ERROR", "destination must be a valid email address.", [
      { field: "destination", message: "destination must be a valid email address." }
    ]);
  return {
