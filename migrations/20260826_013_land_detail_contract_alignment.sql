...
ALTER TABLE land.property_land_details
  ...
  DROP CONSTRAINT IF EXISTS property_land_details_dimension_unit_check,
  DROP CONSTRAINT IF EXISTS property_land_details_road_type_check,
...
BEGIN
  ...
  IF EXISTS (
    ...
    WHERE table_schema = 'land'
      AND table_name = 'property_land_details'
      AND column_name = 'road_type_id'
  ) AND to_regclass('land.road_types') IS NOT NULL THEN
    EXECUTE $sql$
      UPDATE land.property_land_details details
